...
// (13px title vs 15px, 5px vs 6px close padding) — a deliberate context-driven
// difference, not drift, so it keeps its own header markup rather than using this.
// Both close buttons are icon-only (lucide X): the label moves to aria-label so
// screen readers still get it, matching EditorShell's icon-toolbar-button pattern.
export function OverlayPanelHeader({ title, onClose }: { title: ReactNode; onClose: () => void }) {
  ...
  return (
    <div className="overlay-panel__header">
      ...
      <button className="btn overlay-panel__close" onClick={onClose} aria-label={t("actions.close")}>
        <X size={16} />
      </button>
    </div>
  );
