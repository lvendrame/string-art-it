    ...
    sections: [
      {
        ...
        items: [
          ...
          { label: "Move", description: "Press-drag-release to translate the selected Pin Path. Disabled unless a Pin Path is selected; commits as one undo step. Arrow keys also nudge it: 1 screen-pixel of movement per press, 10x with Shift held." },
          { label: "Rotation", description: "Press-drag-release to rotate the selected Pin Path about the point where the mouse was pressed, 0.3° per screen pixel (drag right to increase). Disabled unless a Pin Path is selected; commits as one undo step. Arrow keys also rotate it about the shape's own centre: 1°/10° (Shift) per press, Up/Left anticlockwise, Down/Right clockwise." },
          { label: "Scale", description: "Press-drag-release to resize the selected Pin Path about its own centre (drag right to grow, left to shrink). Recalculates pin count and reattaches any connected threads to the nearest new pin. Disabled unless a Pin Path is selected; commits as one undo step. Arrow keys also scale it: 1%/10% (Shift) per press, Up/Right grows, Down/Left shrinks." },
    ...
    sections: [
      {
        ...
        items: [
          ...
          { label: "ArrowLeft", description: "Thread Draw tool: retracts the last point of the in-progress thread draft. Edit mode with Move/Rotation/Scale active and a Pin Path selected: nudges/rotates/scales it (see the Edit tab)." },
          { label: "ArrowRight", description: "Thread Draw tool: once the draft has 4+ vertices, extends it by extrapolating the numeric pin-position pattern of the vertices added so far, wrapping around the pin count of the relevant Pin Path. Edit mode with Move/Rotation/Scale active and a Pin Path selected: nudges/rotates/scales it (see the Edit tab)." },
          { label: "ArrowUp / ArrowDown", description: "Edit mode with Move/Rotation/Scale active and a Pin Path selected: nudges/rotates/scales it. No effect elsewhere." },
          { label: "Shift (held with an Edit-mode arrow key)", description: "Multiplies the arrow key's Move/Rotation/Scale step by 10." },
          { label: "Holding an Edit-mode arrow key", description: "Repeats the Move/Rotation/Scale step automatically after a 500ms delay, then every 100ms until released." },
